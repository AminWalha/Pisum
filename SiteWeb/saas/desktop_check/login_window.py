...
class LoginWindow(ctk.CTk):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(card, text="AI-assisted radiology reporting",
        ...
        # ── Password ──────────────────────────────────────────────────────────
        ctk.CTkLabel(card, text="Password", font=("Segoe UI", 12),
                     text_color=TEXT_2, anchor="w").pack(fill="x", padx=32)
    ...
    def _set_loading(self, loading: bool):
        if loading:
            self._btn.configure(text="Signing in…", state="disabled",
                                fg_color="#0A6B61")
            self._email.configure(state="disabled")
            self._password.configure(state="disabled")
        else:
            self._btn.configure(text="Sign In", state="normal",
                                fg_color=TEAL)
...
def show_no_access_window(plan_url: str = "https://pisum.app/dashboard.html"):
    ...
    ctk.CTkLabel(card, text="No active subscription",
                 font=("Segoe UI", 15, "bold"), text_color=TEXT_1).pack()
    ctk.CTkLabel(
        card,
        text="Visit pisum.app to activate your access.",
        font=("Segoe UI", 11), text_color=TEXT_2, wraplength=300,
    ).pack(pady=(8, 20))
    ctk.CTkButton(
        card, text="Close", height=38, corner_radius=8,
